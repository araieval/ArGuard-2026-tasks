...
- `label` — binary label for Subtask 1A. `null` only on `dev_test`.
- `fine_grained_label` — multi-label sub-types. Hateful memes draw from the
  hateful taxonomy; non-hateful memes draw from `{Humor, Sarcasm, Other}`.
  Empty list on `dev_test`.
...
python ../format_checker/format_checker.py --subtask 1a \
    --predictions sample_submissions/subtask_1a_sample.tsv
```
